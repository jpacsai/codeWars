...
function splitText(text, max) {
  if (!text) return [];
  const split = text.match(/[\w_.,!"']+/g);
  return split.reduce((parts, cur) => {
      const last = parts[parts.length - 1];
      if (last.length + cur.length + 1 <= max) {
        const [, ...withoutLast] = parts.reverse(); // remove last item without split() or pop()
        return [...withoutLast.reverse(), (last + " " + cur).trim()]; // trim is for first element
      }
      return [...parts, cur];
    },
    [""]
