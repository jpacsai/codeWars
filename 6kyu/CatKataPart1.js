...
function peacefulYard(yard, minDistance) {
    ...
    const pairs = cats.length === 0 ? [] : cats.length === 1 ? [] : cats.length === 2 ? [[0,1]] : [[0, 1], [1, 2], [0, 2]]; 
    for (let i = 0; i < pairs.length; i++) {
      const a = cats[pairs[i][0]];
      const b = cats[pairs[i][1]];
      distance.push(Math.pow(Math.pow(Math.abs(a[0] - b[0]), 2) 
                    + Math.pow(Math.abs(a[1] - b[1]), 2), 0.5));
    }
